...
SELECT * from VW_GetTweetdatawithUser
...
SELECT * FROM VW_GetUserwiseTweetCount
...
------------------------------------
exec MaxFollowersByCollege Khoury;
...
-------------------------------------------------------------------------------------------------
exec MaxTweetByColleges Khoury;
CREATE PROCEDURE MaxTweetByColleges(@collegename AS nvarchar(50))
AS
BEGIN
    SELECT
        Name,
		COUNT(CollegeFollowers) AS FollowersCount
    FROM 
        vw_GetTwitterData
    WHERE
        Name LIKE '%'+@collegename+'%'
...
select dbo.MostActiveVerifiedTwitterUserByName ('Northeastern');
...
select * from vw_GetTwitterData



exec MaxretweetcountByName 'khoury'
...
select dbo.MaxretweetcountByName('Northeastern');
